import { addDoc, collection, deleteDoc, doc, getDoc, onSnapshot, query, Timestamp } from "firebase/firestore"
...
export default function ManagePetListing(){
    ...
    //                      let breedId=petsData[i].breedId;
    //                      // console.log(breedId);
                         
    //                      let breedDoc= await getDoc(doc(db,"breeds", breedId))
    //                      // console.log(breedDoc.data());
    //                      updatePetData.push({...petsData[i], breed:breedDoc.data()})
    //                  }
             ...
             const q=collection(db,"pets")
                ...
                onSnapshot(q,async (petDoc)=>{
                    ...
                    for(let i=0;i<petData.length;i++){
                        ...
                        let exbreed=breedDoc.data()
                        petData[i].breed=exbreed.breedName
                        petData[i].breedType=exbreed.type
                        console.log("pet data",petData);
                        
                    }
                    // console.log(updatePetData);
                    // setPetListing(updatePetData)
                    setPetListing(petData)
                    setLoad(false)
                })
            }
